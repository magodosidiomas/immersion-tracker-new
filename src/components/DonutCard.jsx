...
function DonutCard({ groups = [], centerLabel, title, description, bare = false, horizontal = false, ...props }) {
  ...
  return (
    <div
      ref={cardRef}
      className={`donut-card${bare ? ' donut-card-bare' : ''}${horizontal ? ' donut-card-horizontal' : ''}`}
      {...props}
    >
      {title && (
